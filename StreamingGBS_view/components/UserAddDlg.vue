<template>
    <FormDlg title="添加用户" @hide="onHide" @show="onShow" @submit="onSubmit" ref="dlg" :disabled="errors.any()">
        ...
        <div :class="{'form-group':true,'has-error': errors.has('password')}">
            ...
            <div class="col-sm-7">
                <input type="password" class="form-control" id="input-password" name="password" v-model.trim="form.password" data-vv-as="密码" v-validate="'required'" @keydown.enter="$el.querySelector('#input-password').focus()">
            </div>
        </div>
        ...
        <div :class="{'form-group':true,'has-error': errors.has('usersex')}">
            ...
            <div class="col-sm-7">
                <el-radio v-model.trim="form.usersex" label="1">男</el-radio>
                <el-radio v-model.trim="form.usersex" label="0">女</el-radio>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        async onSubmit() {
            ...
            $.post(this.$store.state.baseUrl + '/user/add', this.form).then(data => {
                if(data.code >= 0){
                    this.$refs['dlg'].hide();
                    this.$emit("submit");
                }else{
