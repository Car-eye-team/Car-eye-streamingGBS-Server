<template>
    <FormDlg title="添加设备" @hide="onHide" @show="onShow" @submit="onSubmit" ref="dlg" :disabled="errors.any()">
        ...
        <div :class="{'form-group':true,'has-error': errors.has('devicetypeid')}">
            ...
            <div class="col-sm-7">
                <el-select style="width:100%;" class="form-control" size="medium" id="input-devicetypeid" name="devicetypeid" data-vv-as="设备类型" v-model.trim="form.devicetypeid" placeholder="请选择" v-validate="'required'">
...
export default {
    ...
    methods: {
        ...
        async onSubmit() {
            ...
            $.post(this.$store.state.baseUrl + '/deviceInfo/add', this.form).then(data => {
                if(data.code >= 0){
                    this.$refs['dlg'].hide();
                    this.$emit("submit");
                }else{
