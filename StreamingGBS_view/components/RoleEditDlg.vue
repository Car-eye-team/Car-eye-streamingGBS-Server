<template>
    <FormDlg title="编辑角色" @hide="onHide" @show="onShow" @submit="onSubmit" ref="dlg" :disabled="errors.any()">
...
export default {
    ...
    methods: {
        ...
        async onSubmit() {
            ...
            $.post(this.$store.state.baseUrl + '/userGroup/edit', this.form).then(data => {
                if(data.code >= 0){
                    this.$refs['dlg'].hide();
                    this.$emit("submit");
                }else{
