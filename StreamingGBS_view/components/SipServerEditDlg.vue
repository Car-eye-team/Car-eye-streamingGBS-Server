<template>
    <FormDlg title="编辑上级平台" @hide="onHide" @show="onShow" @submit="onSubmit" ref="dlg" :disabled="errors.any()" size="modal-lgg">
        <div class="col-sm-6">
...
export default {
    ...
    methods: {
        ...
        async onSubmit() {
            ...
            $.post(this.$store.state.baseUrl + '/sipServer/edit', this.form).then(data => {
                if(data.code >= 0){
                    this.$refs['dlg'].hide();
                    this.$emit("submit");
                }else{
